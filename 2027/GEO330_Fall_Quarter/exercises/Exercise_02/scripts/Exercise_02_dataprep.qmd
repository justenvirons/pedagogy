...
setwd("C:/Users/juste/OneDrive - DePaul University/gitrepos/pedagogy/2027/GEO330_Fall_Quarter")
```
...
adivvy_trips_list <- c("202601-divvy-tripdata","202602-divvy-tripdata","202603-divvy-tripdata", "202604-divvy-tripdata","202605-divvy-tripdata", "202606-divvy-tripdata", "202607-divvy-tripdata",  "202608-divvy-tripdata" )

# unzip monthly and quarterly divvy trip data files downloaded from divvy system website 2020-2026
# https://divvy-tripdata.s3.amazonaws.com/index.html
for (afile in adivvy_trips_list){
  afilename <- paste0("exercises/Exercise_02/data/",afile,".zip")
...
save(divvy_trips_latest_start_ca, divvy_trips_latest_end_ca, divvy_trips_latest_ca_all_updated, file = "exercises/Exercise_02/data/divvy_datatrips_start_end_202608.RData")
...
divvy_trips_latest_ca_all_2013_202608 <- divvy_trips_latest_ca_all_2013_202512 %>% 
...
save(divvy_trips_latest_ca_all_2013_202608, file = "exercises/Exercise_02/data/divvy_trips_latest_ca_all_2013_202608.RData")
...
load("exercises/Exercise_02/data/divvy_trips_latest_ca_all_2013_202608.RData")

assign("divvy_trips_2013_2026_ca_all", divvy_trips_latest_ca_all_2013_202608)

# trips by yearmonth tables
# total trips by year and month
trips_year_month <- divvy_trips_2013_2026_ca_all %>%
  group_by(yearmonth) %>%
  summarize(trips = n(),
            trip_duration = sum(trip_duration))

# total trips by year, month and community area
trips_year_month_fr_ca <- divvy_trips_2013_2026_ca_all %>%
...
trips_year_month_to_ca <- divvy_trips_2013_2026_ca_all %>%
...
# total trips by date
trips_date <- divvy_trips_2013_2026_ca_all %>%
  group_by(date) %>%
  summarize(trips = n(),
            trip_duration = sum(trip_duration))

#total trips by date and community area
trips_date_fr_ca <- divvy_trips_2013_2026_ca_all %>%
  mutate(date_ca = paste0(as.character(date),":",as.character(start_comarea_id))) %>%
  group_by(date_ca) %>%
  summarize(fr_trips = n(),
            fr_trip_duration = sum(trip_duration))

trips_date_to_ca <- divvy_trips_2013_2026_ca_all %>%
...
save(trips_date_ca_all_noduration, trips_yearmonth_ca_all_noduration, divvy_trips_2013_2026_ca_all, file="exercises/Exercise_02/data/divvy_datatrips_ca_sum_latest_2013_202608.RData")
```
...
# average trip duration (minutes per trip) by yearmonth
duration_by_yearmonth <- trips_year_month %>%
  drop_na(yearmonth) %>%
  arrange(yearmonth) %>%
  mutate(yearmonth = ym(yearmonth),
         trip_duration_mins = as.numeric(trip_duration, units = "mins"),
         mins_per_trip = trip_duration_mins / trips) %>%
  select(yearmonth, trips, trip_duration_mins, mins_per_trip)

# average trip duration (minutes per trip) by date
duration_by_date <- trips_date %>%
  drop_na(date) %>%
  mutate(date = as.Date(date, format = "%m/%d/%Y"),
         trip_duration_mins = as.numeric(trip_duration, units = "mins"),
         mins_per_trip = trip_duration_mins / trips) %>%
  arrange(date) %>%
  select(date, trips, trip_duration_mins, mins_per_trip)

# average trip duration (minutes per trip) by yearmonth and community area
# trips from (fr_) and to (to_) each community area; NA where no trips occurred
duration_by_yearmonth_ca <- trips_year_month_ca_all %>%
  drop_na(yearmonth) %>%
  mutate(yearmonth = ym(yearmonth),
         fr_trip_duration_mins = as.numeric(fr_trip_duration, units = "mins"),
         to_trip_duration_mins = as.numeric(to_trip_duration, units = "mins"),
         fr_mins_per_trip = if_else(fr_trips > 0, fr_trip_duration_mins / fr_trips, NA_real_),
         to_mins_per_trip = if_else(to_trips > 0, to_trip_duration_mins / to_trips, NA_real_)) %>%
  arrange(yearmonth, comarea_id) %>%
  select(yearmonth, comarea_id,
         fr_trips, fr_trip_duration_mins, fr_mins_per_trip,
         to_trips, to_trip_duration_mins, to_mins_per_trip)

save(trips_by_date, trips_by_yearmonth,trips_by_yearmonth_ca, duration_by_yearmonth, duration_by_date, duration_by_yearmonth_ca, CARTO_API_KEY, file = "exercises/Exercise_02/data/divvy_trips_summary_tables_latest.RData")
...
community_areas_ccvi %>%
  ...
  st_drop_geometry() %>%
  arrange(ID) %>%
  data.table(class = 'cell-border stripe',
            rownames = FALSE,
